...
def Attention_LSTM_Block(skip_connection, gating_signal, num_filters, multiplier, lstm_multiplier, signal_length):
    ...
    #
    conv1_1 = tf.keras.layers.Reshape(target_shape=(1, np.int32(signal_length / (multiplier * 2)), np.int32(num_filters * multiplier)))(
        skip_connection)
    conv1_1_F = tf.keras.layers.ConvLSTM1D(filters=np.int32(num_filters * lstm_multiplier * 2), kernel_size=3, padding='same', return_sequences=False,
                                           go_backwards=False, kernel_initializer='he_normal')(conv1_1)
    conv1_1_B = tf.keras.layers.ConvLSTM1D(filters=np.int32(num_filters * lstm_multiplier * 2), kernel_size=3, padding='same', return_sequences=False,
                                           go_backwards=True, kernel_initializer='he_normal')(conv1_1)
    #
    ...
    conv1_2_B = tf.keras.layers.ConvLSTM1D(filters=np.int32(num_filters * lstm_multiplier * 2), kernel_size=3, padding='same', return_sequences=False,
                                           go_backwards=True, kernel_initializer='he_normal')(
        merge_B)
    conv = tf.keras.layers.Bidirectional(
        tf.keras.layers.ConvLSTM1D(filters=np.int32(num_filters * lstm_multiplier * 2), kernel_size=3, padding='same', return_sequences=False,
...
def MultiAttention_LSTM_Block(skip_connection, gating_signal, long_skip_connections, num_filters, multiplier, lstm_multiplier, signal_length):
    ...
    conv1_2 = tf.keras.layers.ConvLSTM1D(filters=np.int32(num_filters * lstm_multiplier), kernel_size=3, padding='same', return_sequences=False,
        ...
        for i in range(1, (self.model_depth + 1)):
            for j in range(0, (self.model_depth - i + 1)):
                if (i == 1) and (j == (self.model_depth - 1)):
                    ...
                    if self.LSTM == 1:
                        x1 = tf.keras.layers.Reshape(target_shape=(1, np.int32(self.length / 2 ** j), np.int32(self.model_width * (2 ** j))))(
                            skip_connection)
                        ...
                        deconv = tf.keras.layers.ConvLSTM1D(filters=np.int32(self.model_width * (2 ** (j - 1))), kernel_size=3, padding='same',
        ...
        for i in range(1, (self.model_depth + 1)):
            for j in range(0, (self.model_depth - i + 1)):
                if (i == 1) and (j == (self.model_depth - 1)):
                    ...
                    skip_connection = convs_list[j]
                    if self.A_G == 1:
                        skip_connection = MultiAttention_LSTM_Block(convs_list[j], deconv, deconv_tot, self.model_width, 2 ** j, 2 ** (j - 1),
                                                                    self.length)
                    deconv = Concat_Block(deconv, deconv_tot, skip_connection)
                    deconv = Conv_Block(deconv, self.model_width, self.kernel_size, 2 ** j)
                    deconv = Conv_Block(deconv, self.model_width, self.kernel_size, 2 ** j)
...
def build_NABNet():
    length = 256  # Length of each Segment
    ...
    #
    Model = NABNet(length, model_depth, num_channel, model_width, kernel_size, problem_type=problem_type, output_nums=output_nums,
                   ds=D_S, ae=A_E, ag=A_G, is_transconv=is_transconv).NABNetV1()
